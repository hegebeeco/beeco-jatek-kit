...
function cmp(a, b, p, f){
  ...
  else{ if(ACC.test(b) && b === a && a.length > 12) warns.push(`${f}: ${p} fordítatlannak tűnik: ${a.slice(0, 60)}`);
    const na = (a.match(/\d+(?:[.,]\d+)?/g) || []).map(x => x.replace(',', '.')), nb = (b.match(/\d+(?:[.,]\d+)?/g) || []).map(x => x.replace(',', '.'));
